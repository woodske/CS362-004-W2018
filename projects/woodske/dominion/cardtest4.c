...
int main() {
	...
	// change player's hand to great_hall, estate, gold
	...
	printf("Expected Post-%s:\t handcount = %d, discard = %d, deck = %d, actions = %d\n", TESTCARD, preHandCount, preDiscardCount + 1, preDeckCount - 1, preActionCount);

	if (postHandCount == preHandCount && postDiscardCount == preDiscardCount + 1 && postDeckCount == preDeckCount - 1 && postActionCount == preActionCount) {
