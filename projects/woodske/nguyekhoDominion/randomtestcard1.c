...
int main() {
  printf("\n______________Village Test Begin_________________\n");
  ...
  for (i = 0; i < NUM_TESTS; i++) {
    ...
    x = initializeGame(numPlayer, k, randSeed, &G);         // initialize a new game
    ...
    printf("\nTesting case %d of %d\n", i+1, NUM_TESTS);
		...
		//If there are no cards in deck/discard, no card should be added to hand
    ...
    if (preHandCount == postHandCount - cardMod &&
  ...
  printf("______________Village Test End____________________\n\n");
  return 0;
}